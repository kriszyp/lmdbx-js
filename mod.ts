...
//console.log({version});
let libPath = import.meta.url.startsWith('file:') && fileURLToPath(new URL('build/Release/lmdbx.node', import.meta.url));
if (!libPath || !exists(libPath)) {
    //console.log({ libPath }, 'does not exist')
    libPath = (Deno.env.get('LMDBX_LIB_PATH') || (tmpdir() + '/lmdbx-js-' + (version || '') + '.lib')) as string;
    const ARCH = { x86_64: 'x64', aarch64: 'arm64' }
    if (!exists(libPath)) {
        let os: string = Deno.build.os;
        os = os == 'windows' ? 'win32' : os;
        os += '-' + ARCH[Deno.build.arch];
        let libraryUrl = 'https://cdn.jsdelivr.net/npm/lmdbx@' + (version || 'latest') +
...
let lmdbxLib = Deno.dlopen(libPath, {
	...
	envOpen: { parameters: ['u32', 'u32', 'pointer', 'pointer', 'f64', 'u32', 'u32', 'f64', 'u32'], result: 'i64'},
...
setNativeFunctions({ Env, Compression, Cursor, getAddress, lmdbxError, setGlobalBuffer });
